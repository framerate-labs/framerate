...
export default function TiersPage({ result }: { result: ListResourceProduct }) {
  ...
  const [proTier] = plans.filter(
    (plan) => plan.name.toLowerCase() === "pro pass",
  );

  const [premiereTier] = plans.filter(
    (plan) => plan.name.toLowerCase() === "premiere pass",
  );
      ...
      <div className="relative">
        <div className="absolute left-0 right-0 top-[26px] z-10 mx-auto flex w-fit items-center justify-center gap-x-2 rounded-full border border-white/10 bg-background-darker px-5 py-2 font-semibold tracking-wide">
          ...
          <Switch
            ...
            className="data-[state=checked]:bg-[#441aff] data-[state=unchecked]:bg-[#ed4b00]"
          />
